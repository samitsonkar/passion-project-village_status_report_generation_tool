...
# ================== CONFIGURATION ==================
TARGET_STATE_VAL = "26"      # Punjab
TARGET_DISTRICT_VAL = "382"  # Bathinda
TARGET_BLOCK_VAL = "7075"    # Bathinda Block
# ===================================================
...
def force_select_via_js(driver, dropdown_partial_id, target_value, sleep_time=2):
    ...
    """)
    if success:
        time.sleep(sleep_time) # Wait for background AJAX to finish
    return success
...
def extract_jjm_metrics(driver):
    ...
    # 1. EXTRACT ABSTRACT TEXT DATA
    # Added \s* everywhere to catch hidden spaces, tabs, or newlines around colons
    patterns = {
    ...
    for key, pattern in patterns.items():
        ...
        if match:
            ...
            data[key] = "N/A" # Fills blank if the website genuinely misses a field
...
def main():
    ...
    # Map the IDs to human-readable names for the final CSV
    state_name = "Punjab"
    ...
    try:
        ...
        # 1. One-time Setup: Drill down to Block level
        print("Setting State, District, and Block...")
        force_select_via_js(driver, "state", TARGET_STATE_VAL, sleep_time=3)
        force_select_via_js(driver, "district", TARGET_DISTRICT_VAL, sleep_time=3)
        force_select_via_js(driver, "block", TARGET_BLOCK_VAL, sleep_time=4)

        panchayats = get_dropdown_options_via_js(driver, "panchayat")
        print(f"✅ Found {len(panchayats)} Panchayats. Initiating high-speed extraction...\n")
        ...
        for gp in panchayats:
            ...
            force_select_via_js(driver, "panchayat", gp_val, sleep_time=3)
            villages = get_dropdown_options_via_js(driver, "village")
            
            if not villages:
                print(f"  [!] No villages found for GP: {gp_text}")
                continue
            ...
            for vill in villages:
                ...
                # FAST HACK: Just swap the village value and click submit!
        ...
        if all_data:
            ...
            output_file = f"jjm_data_{block_name}.csv"
            df.to_csv(output_file, index=False)
            print(f"\n✅ SUCCESS: Fast-Scraped {len(all_data)} villages.")
